...
class Program
{
    private static LibraryServices _libraryServices;
    ...
    static async Task Main(string[] args)
    {
        ...
        // Subscribe to events
        _libraryServices.BookBorrowed += OnBookBorrowed;
        _libraryServices.BookReturned += OnBookReturned;
        _libraryServices.MemberRegistered += OnMemberRegistered;
    ...
    static async Task AddBookAsync()
    {
        ...
        try
        {
            ...
            await _libraryServices.AddBookAsync(book);
    ...
    static async Task SearchBooksAsync()
    {
        ...
        try
        {
            var books = await _libraryServices.SearchBookAsync(searchTerm);
    ...
    static async Task ListAvailableBooksAsync()
    {
        ...
        try
        {
            var books = await _libraryServices.GetAvailableBookAsync();
    ...
    static async Task RegisterMemberAsync()
    {
        ...
        try
        {
            ...
            await _libraryServices.RegisterMemberAsync(member);
    ...
    static async Task ViewMemberDetailsAsync()
    {
        ...
        try
        {
            var member = await _libraryServices.GetMemberByNumberAsync(membershipNumber);
    ...
    static async Task BorrowBookAsync()
    {
        ...
        try
        {
            ...
            var availableBooks = await _libraryServices.GetAvailableBookAsync();
            ...
            foreach (var book in availableBooks.Take(10)) // Show first 10
            {
                Console.WriteLine($"ID: {book.Id} | {book.Title} by {book.Author?.FullName}");
            }
            ...
            var loan = await _libraryServices.BorrowBookAsync(membershipNumber, bookId);
    ...
    static async Task ReturnBookAsync()
    {
        ...
        try
        {
            ...
            if (method == "1")
            {
                ...
                var loan = await _libraryServices.ReturnBookAsync(loanId);
            ...
            else if (method == "2")
            {
                ...
                var returnedLoan = await _libraryServices.ReturnBookAsync(selectedLoanId);
                ConsoleHelper.WriteSuccess("Book returned successfully!");
    ...
    static async Task ViewOverdueLoansAsync()
    {
        ...
        try
        {
            var overdueLoans = await _libraryServices.GetOverdueLoanAsync();
            ...
            if (overdueLoans.Any())
            {
                ...
                var totalFines = overdueLoans.Sum(l => l.CalcualteFine());
        ...
        try
        {
            ...
            if (activeLoans.Any())
            {
                ...
                foreach (var loan in activeLoans)
                {
                    ...
                    if (loan.IsOverdue)
                    {
                        ConsoleHelper.WriteWarning($"⚠️ OVERDUE - Fine: {loan.CalcualteFine():C}");
                    }
                    else
                    {
    ...
    static async Task ShowBookStatisticsAsync()
    {
        ...
        try
        {
            var stats = await _libraryServices.GetBookStatisticsAsync();
    ...
    static async Task ShowFinancialReportAsync()
    {
        ...
        try
        {
            ...
            // Collected fines (from returned loans)
    ...
    static async Task AddSampleDataAsync()
    {
        ...
        foreach (var book in books)
        {
            try { await _libraryServices.AddBookAsync(book); }
        ...
        foreach (var member in members)
        {
            try { await _libraryServices.RegisterMemberAsync(member); }
            catch { /* Ignore if already exists */ }
        }
    }
    #endregion
