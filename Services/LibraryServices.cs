    ...
    public class LibraryServices : ILibraryService
    {
        ...
        public async Task<Loan> BorrowBookAsync(string membershipNumber, int bookId)
        {
            ...
            if (activeLoans.Count >= maxBookLimit)
                throw new InvalidOperationException($"Member has reached maximum book limi ({maxBookLimit})!");
